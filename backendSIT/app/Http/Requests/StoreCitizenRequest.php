...
class StoreCitizenRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'id_family' => ['nullable', 'uuid', 'exists:family,id_family'],
            'nama_lengkap' => ['required', 'string', 'max:150'],
            'hubungan_keluarga' => ['nullable', Rule::in(['KEPALA_KELUARGA', 'ISTRI', 'ANAK', 'LAINNYA'])],
            'tempat_lahir' => ['nullable', 'string', 'max:100'],
            'tanggal_lahir' => ['nullable', 'date'],
            'jenis_kelamin' => ['required', Rule::in(['L', 'P'])],
            'id_agama' => ['nullable', 'uuid', Rule::exists('master_data', 'id_master')->where('tipe', 'AGAMA')],
            'status_nikah' => ['nullable', Rule::in(['BELUM_KAWIN', 'KAWIN', 'CERAI_HIDUP', 'CERAI_MATI'])],
            'id_pendidikan' => ['nullable', 'uuid', Rule::exists('master_data', 'id_master')->where('tipe', 'PENDIDIKAN')],
            'id_profesi' => ['nullable', 'uuid', Rule::exists('master_data', 'id_master')->where('tipe', 'PROFESI')],
            ...
            'id_wilayah' => ['required', 'uuid', 'exists:wilayah,id_wilayah'],
